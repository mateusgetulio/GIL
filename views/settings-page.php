...
<div class="wrap">
	...
	<div class="gil-status-line">
		...
		<span>
			...
			if ( $enabled ) {
				...
				esc_html_e( 'Waiting room is off. Visitors are not being limited.', 'get-in-line' );
			}
			?>
		</span>
		<a class="gil-preview-link" href="<?php echo esc_url( $preview_url ); ?>" target="_blank" rel="noopener"><?php esc_html_e( 'Preview the waiting room', 'get-in-line' ); ?></a>
	</div>
	...
	<?php if ( $enabled ) : ?>
		<div class="gil-stat-cards">
			<div class="gil-stat-card">
				...
				<div class="gil-stat-card-label"><?php esc_html_e( 'Currently admitted', 'get-in-line' ); ?></div>
			</div>
			<div class="gil-stat-card">
				...
				<div class="gil-stat-card-label"><?php esc_html_e( 'Waiting in line', 'get-in-line' ); ?></div>
			</div>
		</div>
		<p class="gil-stat-meta"><?php esc_html_e( 'Auto-updates every 10 seconds.', 'get-in-line' ); ?> <span id="gil-updated"></span></p>
		<p class="gil-screen-reader-only" role="status" id="gil-admin-live-status"></p>
	<?php else : ?>
		<p class="gil-status-disabled-note"><?php esc_html_e( 'Live status appears here when the waiting room is on.', 'get-in-line' ); ?></p>
	<?php endif; ?>
	...
	<h2><?php esc_html_e( 'Clear the queue', 'get-in-line' ); ?></h2>
	...
	$get_in_line_confirm_message = sprintf(
		...
		__( 'Remove %1$s admitted sessions and %2$s waiting visitors? They will rejoin from scratch.', 'get-in-line' ),
